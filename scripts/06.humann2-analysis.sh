...
# A script to perform humann2 analyis for function annotation using shogun reads while 
...
OUT_DIR="${PROJECT_DIR}/04.read_based_analysis/05.humann2-classify/"
...
parallel --jobs 5  "humann2 \
			...
			--threads ${THREADS} \
			--output ${OUT_DIR}/{} \
			--input ${RAW_DATA_DIR}/{}_${SUFFIX}.${INPUT_FORMAT}"  ::: ${SAMPLES[*]}
...
merge_metaphlan_tables.py ${OUT_DIR}/bugs_list/*  > ${OUT_DIR}/metaphlan2_merged.tsv

# To read this table into STAMP or R convert it be a .SPF file with this command:
${METAPHLAN2STAMP}  ${OUT_DIR}/metaphlan2_merged.tsv  > ${OUT_DIR}/metaphlan2_merged.spf
...
sed -i "s/${SUFFIX}_metaphlan_bugs_list//g" ${OUT_DIR}/metaphlan2_merged.spf
...
humann2_join_tables --input ${OUT_DIR}/pathway_abund_tables/ --file_name pathabundance --output  ${OUT_DIR}/humann2_pathabundance.tsv
humann2_split_stratified_table --input ${OUT_DIR}/humann2_pathabundance.tsv --output ${OUT_DIR} 
sed "s/${SUFFIX}_Abundance//g" ${OUT_DIR}/humann2_pathabundance_unstratified.tsv  >  ${OUT_DIR}/humann2_pathabundance_unstratified.spf
sed -i 's/# Pathway/MetaCyc_pathway/' ${OUT_DIR}/humann2_pathabundance_unstratified.spf

# Normalized counts
# Normalize each sample into relative abundance (so that the counts for each sample sum to 100)
humann2_renorm_table --input ${OUT_DIR}/humann2_pathabundance.tsv --units relab --output ${OUT_DIR}/humann2_pathabundance_relab.tsv
# Unstratify the pathway
humann2_split_stratified_table --input ${OUT_DIR}/humann2_pathabundance_relab.tsv --output ${OUT_DIR}
sed "s/${SUFFIX}_Abundance//g" ${OUT_DIR}/humann2_pathabundance_relab_unstratified.tsv  >  ${OUT_DIR}/humann2_pathabundance_relab_unstratified.spf
sed -i 's/# Pathway/MetaCyc_pathway/' ${OUT_DIR}/humann2_pathabundance_relab_unstratified.spf
...
function join_tables(){
		...
		# Raw tables
                ...
                humann2_join_tables --input  ${OUT_DIR}/functional_groups_tables/ --file_name ${group} --output  ${OUT_DIR}/humann2_${group}.tsv
		humann2_split_stratified_table --input ${OUT_DIR}/humann2_${group}.tsv --output ${OUT_DIR}/
		sed -E "s/${SUFFIX}_Abundance\-RPKs//g"  ${OUT_DIR}/humann2_${group}_unstratified.tsv  \
                >  ${OUT_DIR}/humann2_${group}_unstratified.spf
                sed -i -E "s/# Gene Family/${group}/"  ${OUT_DIR}/humann2_${group}_unstratified.spf
				
		# Normalized tables
                # Normalize each sample into relative abundance (so that the counts for each sample sum to 100)
                humann2_renorm_table --input  ${OUT_DIR}/humann2_${group}.tsv --units relab --output  ${OUT_DIR}/humann2_${group}_relab.tsv
                humann2_split_stratified_table --input ${OUT_DIR}/humann2_${group}_relab.tsv --output ${OUT_DIR}/
                # Using sed to make string replacements we can also format the header-line for input into STAMP.
                sed -E "s/${SUFFIX}_Abundance\-RPKs//g"  ${OUT_DIR}/humann2_${group}_relab_unstratified.tsv  \
                >  ${OUT_DIR}/humann2_${group}_relab_unstratified.spf
                sed -i -E "s/# Gene Family/${group}/"  ${OUT_DIR}/humann2_${group}_relab_unstratified.spf
...
function join_named_tables(){
	...
	for group in ${RENAME_GROUPS[*]}; do
                ...
                humann2_join_tables --input  ${OUT_DIR}/renamed_groups_tables/ --file_name ${group}_names --output ${OUT_DIR}/humann2_${group}_names.tsv
		humann2_split_stratified_table --input ${OUT_DIR}/humann2_${group}_names.tsv --output ${OUT_DIR}/

		 # Using sed to make string replacements we can also format the header-line for input into STAMP.
                sed -E  "s/${SUFFIX}_Abundance\-RPKs//g"  ${OUT_DIR}/humann2_${group}_names_unstratified.tsv  \
                >  ${OUT_DIR}/humann2_${group}_names_unstratified.spf
                sed -i "s/# Gene Family/${group}/"  ${OUT_DIR}/humann2_${group}_names_unstratified.spf
		

                # Normalize each sample into relative abundance (so that the counts for each sample sum to 100)
                humann2_renorm_table --input  ${OUT_DIR}/humann2_${group}_names.tsv --units relab --output  ${OUT_DIR}/humann2_${group}_names_relab.tsv
                humann2_split_stratified_table --input ${OUT_DIR}/humann2_${group}_names_relab.tsv --output ${OUT_DIR}/
                # Using sed to make string replacements we can also format the header-line for input into STAMP.
                sed -E  "s/${SUFFIX}_Abundance\-RPKs//g"  ${OUT_DIR}/humann2_${group}_names_relab_unstratified.tsv  \
                >  ${OUT_DIR}/humann2_${group}_names_relab_unstratified.spf
                sed -i "s/# Gene Family/${group}/"  ${OUT_DIR}/humann2_${group}_names_relab_unstratified.spf
...
humann2_join_tables --input  ${OUT_DIR}/gene_family_tables/ --file_name genefamilies --output   ${OUT_DIR}/humann2_genefamilies.tsv
humann2_split_stratified_table --input ${OUT_DIR}/humann2_genefamilies.tsv --output ${OUT_DIR}/
# Using sed to make string replacements we can also format the header-line for input into STAMP.
sed "s/${SUFFIX}_Abundance\-RPKs//g"  ${OUT_DIR}/humann2_genefamilies_unstratified.tsv  >  ${OUT_DIR}/humann2_genefamilies_unstratified.spf
sed -i 's/# Gene Family/Uniref_gene_family/'  ${OUT_DIR}/humann2_genefamilies_unstratified.spf

# Normalized tables
# Normalize each sample into relative abundance (so that the counts for each sample sum to 100)
humann2_renorm_table --input  ${OUT_DIR}/humann2_genefamilies.tsv --units relab --output  ${OUT_DIR}/humann2_genefamilies_relab.tsv
...
humann2_split_stratified_table --input ${OUT_DIR}/humann2_genefamilies_relab.tsv --output ${OUT_DIR}/
...
# Using sed to make string replacements we can also format the header-line for input into STAMP.
sed "s/${SUFFIX}_Abundance\-RPKs//g"  ${OUT_DIR}/humann2_genefamilies_relab_unstratified.tsv  >  ${OUT_DIR}/humann2_genefamilies_relab_unstratified.spf
sed -i 's/# Gene Family/Uniref_gene_family/'  ${OUT_DIR}/humann2_genefamilies_relab_unstratified.spf
